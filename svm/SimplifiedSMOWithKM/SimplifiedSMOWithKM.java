 ...
 */
public class SimplifiedSMOWithKM {

    public double[] E; //存储E
    public int[] indexOpt = new int[2]; //优化的参数的索引
    KernelFunction K;
    SVMModel model;
    int maxEpoch = 0; //变量未更新的次数
    public boolean[] selectMask; //变量是否已被更新过
    double epsilon;



    public SimplifiedSMOWithKM(double[][] x, int[] y, double C, KernelFunction.kernel k, double epsilon, int maxEpoch){
        /*构造函数
        * */
        //init alpha and b
    ...
    public SimplifiedSMOWithKM(double[][] x, int[] y, double C, KernelFunction.kernel k, double parameter, double epsilon, int maxEpoch){
        /*构造函数
        * */
        //init alpha and b
        this.E = new double[y.length];
    ...
    public boolean boundCondi(){
        ...
        for (int i = 0; i < model.alpha.length; i++) {
            if (model.alpha[i] > -epsilon && model.alpha[i] < model.C + epsilon){
                flag = false;
                break;
            }
        }
    ...
    public void updateParamter(){
        ...
        String key12  = (indexOpt[0] < indexOpt[1]) ? ( indexOpt[0] + "s" + indexOpt[1]) : ( indexOpt[1] + "s" + indexOpt[0]);
        double K11;
        double K12;
        double K22;
        if (model.kernelMatrix.containsKey(key12)){
            K12 = model.kernelMatrix.get(key12);
        }else{
            K12 = K.calculateK(model.x[indexOpt[0]], model.x[indexOpt[1]]);
            model.kernelMatrix.put(key12, K12);
        }
        if (model.kernelMatrix.containsKey(indexOpt[0] + "s" + indexOpt[0])){
            K11 = model.kernelMatrix.get(indexOpt[0] + "s" + indexOpt[0]);
        }else{
            K11 = K.calculateK(model.x[indexOpt[0]], model.x[indexOpt[0]]);
            model.kernelMatrix.put(indexOpt[0] + "s" + indexOpt[0], K11);
        }
        if (model.kernelMatrix.containsKey(indexOpt[1] + "s" + indexOpt[1])){
            K22 = model.kernelMatrix.get(indexOpt[1] + "s" + indexOpt[1]);
        }else{
            K22 = K.calculateK(model.x[indexOpt[1]], model.x[indexOpt[1]]);
            model.kernelMatrix.put(indexOpt[1] + "s" + indexOpt[1], K22);
        }
        ...
        // 更新b
        calculateB(model.alpha[indexOpt[0]], alpha1Old, model.alpha[indexOpt[1]], alpha2Old, K11, K12, K22 );
    ...
    public static void main(String[] args){
        ...
        SimplifiedSMOWithKM smo = new SimplifiedSMOWithKM(reader.x, reader.y , 1, k, 0.01, 0.001, 5000);
